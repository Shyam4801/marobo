...
class Rollout(BO_Interface):
    ...
    def sample(
        ...
        for sample in tqdm(range(num_samples)):
            ...
            agents_to_subregion = self.get_subregion(deepcopy(X_root), num_agents, dim=0)
            ...
            agents = [Agent(model, xtr, ytr) for _ in range(num_agents)]
            # for i in agents:
            ...
            for agent_idx, agent in enumerate(agents):
                rollout_samples = lhs_sampling(5, self.internal_region_support, tf_dim, rng)
                print('below rollout sample: ', self.internal_region_support)
                ...
                agent_predictions = self.get_most_min_sample(agents, self.internal_region_support, rng)
                ...
                self.internal_region_support, assignments, not_inside_any = self.modify_subregion(agent.point_history[-1], agents_to_subregion, internal_inactive_subregion, assignments, agent_idx)
            ...
            for i in assignments.keys():
                ...
                if assignments[i] > 1:
                    ...
                    ch = self.get_subregion(deepcopy(i), assignments[i])
                    i.add_child(ch)
    ...
    def modify_subregion(self, sample_point, agents_to_subregion, inactive_subregion, assignments, agent):
        ...
        for i, subregion in enumerate(agents_to_subregion):
            ...
            if inside: 
                ...
                agents_to_subregion[agent].status = 0

        if agent == 0 and inside:
            fall_under = []
        ...
        if not_inside_any:
            ...
            print('b4 hstack shapes: ',region_support[0].shape, fall_under[0].shape)
            region_support = np.hstack((region_support,fall_under))
        ...
        return region_support, assignments, not_inside_any
    ...
    def rollout_acquisition(self, agent, gpr_model, rollout_samples, test_function, behavior, rng):
        ...
        for smp in rollout_samples:
            ...
            pred_sample_x, min_bo_val = self._opt_acquisition(ytr, model, self.region_support, rng)
    ...
    def get_most_min_sample(self, agent_posterior, region_support, rng):
        agent_predictions = []
        print('region support inside get_most_min_sample: ',region_support)
        for agent in agent_posterior:
            predx, min_bo_val = self._opt_acquisition(agent.y_train, agent.model, region_support, rng) 
